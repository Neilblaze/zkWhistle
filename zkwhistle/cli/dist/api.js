import { Verifier, witnesses } from '@midnight-ntwrk/zkwhistle-contract';
...
export const deploy = async (providers, ownerSk) => {
...
export const addIssuer = async (providers, verifierContract, ownerSk, issuerKey) => {
...
export const revokeIssuer = async (providers, verifierContract, ownerSk, issuerKey) => {
    logger.info('Updating private state with owner secret key for authentication...');
    const currentPrivateState = await providers.privateStateProvider.get('verifierPrivateState');
    if (!currentPrivateState) {
        throw new Error('Could not find private state to update for owner authentication.');
...
export const getChallenge = async (verifierContract) => {
...
export const submitHealthProof = async (verifierContract, challenge, issuerKey) => {
...
}), Rx.map((s) => s.balances[nativeToken()] ?? 0n), Rx.filter((balance) => balance > 1000000n)));
