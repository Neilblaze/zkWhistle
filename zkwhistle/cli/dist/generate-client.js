...
async function showStartupAnimation() {
    ...
    console.log(chalk.green.bold('\nzkWhistle Secure Credential Generator'));
}
// --- Main Tool Logic ---
async function main() {
    ...
    try {
        ...
        const hashPromise = execAsync(`node src/hash-vc.mjs ${ownerSkHex} ${labResults.cholesterol} ${labResults.bloodPressure} ${labResults.isSmoker}`);
        ...
        await fs.writeFile(outputFilename, JSON.stringify(credential, (key, value) => typeof value === 'bigint' ? value.toString() : value, 2));
