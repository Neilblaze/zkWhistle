...
export class ReportStorageService {
  private static readonly STORAGE_KEY = 'zkwhistle_reports';
  private static readonly VOTES_KEY = 'zkwhistle_votes';

  /**
   * Store a new report
   */
  static storeReport(reportData: {
    title: string;
    content: string;
    attachment?: string | null;
    attachmentName?: string | null;
   ...
   */
  static updateReportStatus(reportId: string, status: "pending" | "reviewed" | "archived"): void {
    const reports = this.getAllReports();
   ...
   * Vote on a report
   */
  static voteOnReport(reportId: string, voteType: "up" | "down", walletAddress: string): void {
    const reports = this.getAllReports();
    const reportIndex = reports.findIndex(r => r.id === reportId);
   ...
   * Save user's votes
   */
  private static saveUserVotes(walletAddress: string, votes: Record<string, "up" | "down">): void {
    try {
      const allVotes = localStorage.getItem(this.VOTES_KEY);
      const votesData = allVotes ? JSON.parse(allVotes) : {};
